...
function deriveTurnStatus(turn: ImageTurn): Pick<ImageTurn, "status" | "error"> {
  ...
  if (failedCount > 0) {
    return { status: "error", error: `${failedCount} images failed to generate` };
  }
  if (successCount > 0) {
    ...
    const turns = conversation.turns.map((turn) => {
      ...
      const images = turn.images.map((image) => {
        ...
        return {
          ...
          error: "Page refresh or task interruption; no recoverable task ID found",
        };
      });
...
function ImagePageContent({ isAdmin }: { isAdmin: boolean }) {
  ...
  const deleteConfirmDescription =
    deleteConfirm?.type === "all"
      ...
      : deleteConfirm?.type === "prompt"
        ...
        : deleteConfirm?.type === "results"
          ? "Delete these generated results? The prompt record will be kept."
          : deleteConfirm?.type === "one"
  ...
  const handleDeleteTurnPart = async (conversationId: string, turnId: string, part: "prompt" | "results") => {
    ...
    const turns = conversation.turns
      .map((turn) => {
        ...
        const images =
          part === "results"
            ? turn.images.map((image) => ({ id: image.id, status: "error" as const, error: "Generated results deleted" }))
            : turn.images;
        const derived =
          part === "results"
            ? deriveTurnStatus({
  ...
  const handleContinueEdit = useCallback(
    async (conversationId: string, image: StoredImage | StoredReferenceImage) => {
      try {
        ...
        setImagePrompt("");
        textareaRef.current?.focus();
        toast.success("Reference image added. Describe the edit to continue");
      } catch (error) {
        const message = error instanceof Error ? error.message : "Failed to load result image";
        toast.error(message);
      }
    },
    [],
    ...
    async (conversationId: string) => {
      ...
      try {
        ...
        if (activeTurn.mode === "edit" && referenceFiles.length === 0) {
          throw new Error("No reference image available to continue editing");
        }
  ...
  const handleTimeoutRetryContinue = useCallback(async () => {
    ...
    try {
      ...
      toast.info(`Still waiting for ${imageTimeoutRetrySecs} seconds`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Failed to resume polling";
      toast.error(msg);
      setTimeoutRetry(null);
    }
