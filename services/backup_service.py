...
def _openssl_encrypt(data: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        raise BackupError("openssl is missing, unable to perform encrypted backup") from exc
...
def _openssl_decrypt(data: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        raise BackupError("openssl is missing, unable to decrypt backup content") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or b"").decode("utf-8", errors="replace").strip()
        raise BackupError(f"Backup decryption failed: {detail or 'openssl execution failed'}") from exc
    return result.stdout
...
class BackupService:
    ...
    def download_backup(self, key: str) -> dict[str, object]:
        ...
        if candidate.endswith(".enc"):
            ...
            if not passphrase:
                raise BackupError("Encryption passphrase not configured, unable to download and decrypt encrypted backup")
    ...
    def _decode_backup_payload(self, key: str, payload: bytes) -> dict[str, object]:
        ...
        if key.endswith(".enc"):
            passphrase = _clean(config.get_backup_settings().get("passphrase"))
            if not passphrase:
                raise BackupError("Encryption passphrase not configured, unable to view encrypted backup")
    ...
    def _decode_archive_detail(self, payload: bytes) -> dict[str, object]:
        ...
        try:
            ...
            raise BackupError("Failed to parse backup archive, the backup may be corrupted") from exc
