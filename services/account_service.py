...
class AccountService:
    ...
    def _normalize_status(value: object) -> str:
        """Normalize account status to the canonical English value.

        Maps legacy Chinese status values (正常/限流/异常/禁用) stored by older
        versions, as well as common English aliases, to the canonical set:
        normal / rate_limited / abnormal / disabled.
        """
        ...
        return aliases.get(raw) or aliases.get(compact) or (raw if raw else "normal")
    ...
    def _record_token_refresh_error(self, access_token: str, event: str, error: str) -> None:
        ...
        log_service.add(
            LOG_TYPE_ACCOUNT,
            "Failed to refresh access_token with refresh_token",
    ...
    def _apply_refreshed_tokens(self, old_access_token: str, token_data: dict, event: str) -> str:
        ...
        log_service.add(
            LOG_TYPE_ACCOUNT,
            "Refreshed access_token with refresh_token",
            ...
            if result.get("ok"):
                ...
                if error_type == "password_verify_failed_403" and isinstance(result.get("detail"), dict):
                    ...
                    if isinstance(detail_error, dict) and detail_error.get("code") == "account_deactivated":
                        ...
                        log_service.add(
                            LOG_TYPE_ACCOUNT,
                            "Account disabled - marked disabled",
                            {
                                "source": event,
                                "token": anonymize_token(access_token),
                                "email": email,
    ...
    def _add_account_payloads(self, payloads: list[dict]) -> dict:
        ...
        with self._lock:
            ...
            log_service.add(LOG_TYPE_ACCOUNT, f"Added {added} account(s), skipped {skipped}",
        ...
        with self._lock:
            ...
            if removed:
                if self._accounts:
                    ...
                    self._index = 0
                self._save_accounts()
                log_service.add(LOG_TYPE_ACCOUNT, f"Deleted {removed} account(s)", {"removed": removed})
            items = [dict(item) for item in self._accounts.values()]
    ...
    def init_refresh_progress(self, progress_id: str, total: int) -> None:
        ...
        with self._refresh_progress_lock:
            self._refresh_progress[progress_id] = {
                ...
                "done": False,
                "error": None,
                "status_counts": {"normal": 0, "rate_limited": 0, "abnormal": 0, "disabled": 0},
                "total_quota": 0,
            }
    ...
    def update_refresh_progress(self, progress_id: str, token: str) -> None:
        ...
        status = str(account.get("status") or "normal").strip() if account else "normal"
    ...
    def get_stats(self) -> dict:
        ...
        total = len(items)
        active = sum(1 for a in items if a.get("status") == "normal")
        ...
        total_quota = sum(max(0, int(a.get("quota") or 0)) for a in items if a.get("status") == "normal")
