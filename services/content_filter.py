...
def check_request(text: str) -> None:
    ...
    for word in config.sensitive_words:
        if word in text:
            raise HTTPException(status_code=400, detail={"error": "Sensitive words detected, task rejected"})
    ...
    def _on_failure(event_payload: dict) -> None:
        ...
        if not fail_open:
            raise HTTPException(
                status_code=503,
                detail={"error": "AI review service temporarily unavailable, please try again later"},
            )
    ...
    if _is_reject_decision(decision):
        raise HTTPException(status_code=400, detail={"error": "AI review failed, task rejected"})
