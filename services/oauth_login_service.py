...
class OAuthLoginService:
    ...
    def _extract_code_from_callback(value: str) -> tuple[str, str]:
        ...
        if raw.startswith("http://") or raw.startswith("https://"):
            try:
                ...
                raise OAuthLoginError(f"Unable to parse callback URL: {exc}") from exc
    ...
    def finish(self, session_id: str, callback: str) -> dict[str, str]:
        ...
        if session is None:
            raise OAuthLoginError(
                "OAuth session has expired or does not exist, please go back to the import dialog and click \"Regenerate\" to try again"
            )

        if state and session.get("state") and state != session["state"]:
            raise OAuthLoginError(
                "State mismatch. Common cause: you clicked \"Open authorization page\" twice, but the browser still has the previous window logged in. Please click \"Regenerate\" and start over."
            )
    ...
    def _exchange_code(code: str, code_verifier: str, redirect_uri: str) -> dict[str, str]:
        ...
        try:
            ...
            raise OAuthLoginError(f"Network error while exchanging token: {exc}") from exc
        finally:
            session.close()
        ...
        if response.status_code != 200 or not isinstance(data, dict) or not data.get("access_token"):
            ...
            raise OAuthLoginError(
                f"OpenAI refused to exchange token (HTTP {response.status_code}){': ' + detail if detail else ''}"
            )
        ...
        if not refresh_token:
            ...
            raise OAuthLoginError(
                "OpenAI did not return refresh_token (scope may not include offline_access or the code may have already been used)"
            )
