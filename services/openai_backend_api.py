...
EDITABLE_FILE_PPT_PROMPT = """Based on the user's requirements, create an editable PPT. You can use agents. Do not keep asking the user questions - you may fill in content style, layout, colors, content structure and page info yourself and execute directly. The overall flow is as follows:
1. Use image generation to create a beautiful product introduction PPT, 5-6 pages
2. Split all the images and shape assets above into separate PNGs, one image per asset, without omissions, so I can reassemble the assets directly in the PPT - no text
3. Using all the images and shape assets above, recreate the showcase PPT you generated first. I need an editable PPT format; the main parts must be inserted separately and text must be editable
Finally just give me a PPT file, plus a zip file of any asset packages encountered during generation."""
EDITABLE_FILE_PSD_PROMPT = "Generate this image, split this poster into several images including the background image, keep the position of every element unchanged so I can use it directly without dragging, background color white, no fake transparency. Then reassemble the split images into a single PSD file, remove the white background, do not change the relative positions of layers, preserve the layer position of each element and keep each element's layer. Finally just output the PSD file plus a zip of each layer."
...
class OpenAIBackendAPI:
    ...
    def get_user_info(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "status": "rate_limited" if quota == 0 else "normal",
        }
        logger.debug({
    ...
    def _editable_prompt(fixed_prompt: str, user_prompt_text: str) -> str:
        ...
        return fixed_prompt if not extra else fixed_prompt + "\n\nThe following is the user's additional requirement, please incorporate it directly:\n" + extra
    ...
    def _poll_image_results(
        ...
        exc = ImagePollTimeoutError(
            f"ChatGPT image generation timed out (waited {timeout_secs}s). "
            f"You can increase image_poll_timeout_secs in config.json, "
            f"or the account may be rate limited or the generation queue may be congested.",
            conversation_id or "",
        )
        if last_task_error:
    ...
    def _iter_sse_payloads_capped(self, response: Any, hard_cap_secs: float) -> Iterator[str]:
        ...
        watchdog.start()
        timeout_message = f"Image generation stream exceeded the hard cap of {int(hard_cap_secs)}s and was forcibly interrupted (upstream may not have generated an image)"
        try:
            for payload in iter_sse_payloads(response):
                yield payload
