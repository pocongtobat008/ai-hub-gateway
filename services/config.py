...
def _normalize_gemini_settings(value: object) -> dict[str, object]:
    source = value if isinstance(value, dict) else {}
    ...
    return {
        ...
        "default_model": str(source.get("default_model") or "").strip(),
    }
...
def _load_settings() -> LoadedSettings:
    ...
    if _is_invalid_auth_key(auth_key):
        raise ValueError(
            "❌ auth-key not set!\n"
            "Set it in the CHATGPT2API_AUTH_KEY environment variable, or fill in auth-key in config.json."
        )
...
class ConfigStore:
    def __init__(self, path: Path):
        ...
        if _is_invalid_auth_key(self.auth_key):
            raise ValueError(
                "❌ auth-key not set!\n"
                "Resolve it in one of the following ways:\n"
                "1. Add an environment variable on Render:\n"
    ...
    def update(self, data: dict[str, object]) -> dict[str, object]:
        ...
        if "gemini" in next_data:
            next_data["gemini"] = _normalize_gemini_settings(next_data.get("gemini"))
    ...
    def get_gemini_settings(self) -> dict[str, object]:
        return _normalize_gemini_settings(self.data.get("gemini"))
