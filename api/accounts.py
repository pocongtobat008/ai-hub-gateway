...
def create_router() -> APIRouter:
    ...
    async def update_user_key(
        ...
        if not updates:
            raise HTTPException(status_code=400, detail={"error": "No changes detected, please modify and save again"})
        try:
    ...
    async def export_accounts(body: AccountExportRequest, authorization: str | None = Header(default=None)):
        ...
        if not items:
            raise HTTPException(
                status_code=400,
                detail={"error": "No complete accounts available for export, need access_token, refresh_token and id_token at the same time"},
            )
